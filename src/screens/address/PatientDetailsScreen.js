...
if (secondaryPhone && secondaryPhone.length !== 10) {
  ...
  const handleSave = async () => {
    ...
    try {
     const data = {
  userId: user?.phone, // better to use phone as userId
  name: `${firstName} ${lastName}`,
  age: parseInt(age, 10),
  email: email || undefined,
  primaryPhone: user?.phone || "",
  secondaryPhone: secondaryPhone || undefined, // ✅ ADD THIS
  gender,
  orderingFor: orderFor === "Myself" ? "myself" : "someone",
};
      await createPatient(data);

      // Navigate to DeliveryDetails if it exists in the stack, otherwise go back
      const state = navigation.getState();
      const hasDelivery = state.routeNames?.includes("DeliveryDetails");
      const hasCart = state.routes.find(r => r.name === "CartHome" || r.name === "Cart");

      if (hasDelivery) {
        navigation.navigate("DeliveryDetails");
      } else if (hasCart) {
        navigation.navigate(hasCart.name);
      } else {
        // Go back two screens (skip AddressDetails)
